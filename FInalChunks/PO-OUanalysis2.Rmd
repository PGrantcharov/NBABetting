...
df <- tidy %>% 
  select(V, H, VF, HF) %>%
  mutate(Bet_V_win = if_else(VF>HF, 95, if_else(VF<HF, -100, 0))) %>%
  mutate(Bet_H_win = if_else(VF<HF, 95, if_else(VF>HF, -100, 0)))
...
ggplot(df_Team, aes(x= profit, y= fct_reorder(Team, profit))) +
  geom_point() +
  labs(y="Betting Profit", x="NBA Team") +
  ggtitle("Bet Same Team Win For Pass 10 years")
