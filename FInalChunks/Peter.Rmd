...
We therefore found this dataset to be an excellent suitor for delivering captivating insights and exercising our data analysis and visualization skills. Further, through this report's analysis, we are particularly interested in answwering the questions of:
...
###3.2.3  Bad Entries:
To located the bad entries, we found it effective to make graphs of the different variables - particularly box plots. This was performed for each of the numerical variables in the dataset:
...
Sports matches, on the other hand, consists of an infinite number of variables which can only be estimated. Since betting on sports outcomes is offered, though, this implies that there may be a discrepancy between the probability of winning certain bets and the TRUE probability (incalculable in theory) of those events occuring. This leaves an opening for potential exploitation.
...
Our most successful betting system pertains to over/under betting for the second half of basketball games. In such a bet, a sportsbook will provide a **number.** The bettor can then bet on whether they think the total number of points scored in the second half of the basketball game will be *over* or *under* this **number.**

Through a gradual evolving of a betting system that built upon previous discoveries, we were able to finalize our very simple, yet effective, betting system. In the first iteration of the discovery process, we proposed a betting system that would pick the over or the under based on prior information. The prior information turned out to be irrelevant, but regardless, please notice the clear separation of the bars in Figure 1, with those on the left half being mostly profitable, and those on the right half being unprofitable.

```{r echo=FALSE, fig.align="center", fig.width = 7, fig.height=4}
df2 <- df1 %>%  group_by(Diff_dec_idx) %>% summarise(profit = sum(earning)) %>%
...
From this graph, we could see that simply changing our strategy such that the red bars would be inverted would result in a very effective system. This betting system is simply to select the "under" for all over/under bets in the second half of NBA games. If \$100 had been made on such a bet for every game since 2007, then Figure 2 would represent the running profit over time since that point.

###5.2.2 Outcomes

```{r echo=FALSE, fig.align="center", fig.width=7, fig.height=4}
winner + labs(caption = "Fig. 2")
```


##5.3 Conclusion
Clearly, these results are significant. They show that our proposed betting system would have made a bettor nearly \$100,000 if it had been implemented since 2007. It is of course impossible to predict future games, however, there cannot be a stronger indicator than past results!
