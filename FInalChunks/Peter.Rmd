...
* bets on the second half "OVER" if the over/under line increased, or;

* bets on the second half "UNDER" if the over/under line decreased, or;
...
tidy <- tidy %>% select(Date, VF, HF, HSpreadClose, VSpreadClose, HSpreadChange, VSpreadChange)

spreadChange <- data_frame("Change" = c(-2.5, -2, -1.5, -1, -0.5, 0.5, 1, 1.5, 2, 2.5))
spreadChange$Profit <- 0

for (j in 1:length(spreadChange$Change)){ 
  tidy$Payout <- 0
  for (i in 1:length(tidy$Payout)) {
    if (spreadChange$Change[j] == tidy$HSpreadChange[i]) {  # if spread change for the game is a hit
      if (tidy$VF[i] - tidy$HF[i] < tidy$HSpreadClose[i]) {    # if home team beat spread
        tidy$Payout[i] <- 95
      ...
      if (tidy$HF[i] - tidy$VF[i] < tidy$VSpreadClose[i]) {    # if vis team beat spread
        tidy$Payout[i] <- 95
...
ggplot(spreadChange, aes(x=Change, y=Profit)) +
  ...
  labs(x="Spread Change", title="Placing a $100 Bet On Sides With Different Spreads at the Open and Close") +   theme_bw()

```

Clearly, this is not an valuable source of information for bettors. Regardless of whether you made your bets in favor or against the direction that the spread line moved, you would walk away a loser at the end of the day. 

###4.2.2 A No Commission World

This result fact, though, gives us a great opportunity to highlight the damaging long term effect of paying a 2.5% commission on every bet, as was done in this simulation. To showcase this, we will present the outcomes of running the exact same simulation, but this time, winning \$100 on a \$100 bet. 
...
ggplot(spreadChange, aes(x=Change, y=Profit)) +
  ...
  labs(x="Spread Change", y="Profit ($)",
       title="Net Profit When Betting in Direction of Given Spread Change") +
  theme_bw()
```
...
for (i in 2:length(tidy$Payout)) {
  if (tidy$HSpreadChange[i] > 0) {  # if spread change for the game is a hit
    if (tidy$VF[i] - tidy$HF[i] < tidy$HSpreadClose[i]) {    # if home team beat spread
