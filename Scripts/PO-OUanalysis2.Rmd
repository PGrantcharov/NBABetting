...
df <- tidy %>% 
  ...
  mutate(Diff_dec_idx = 
           cut(Diff,
               ...
               labels = c("-5","-4","-3", "-2", "-1", "-0.5", "skip -1 to 1", "0.5", "1", "2","3", "4", "5"))) %>%
...
df <- tidy %>% 
  ...
  filter(!(Diff_dec_idx == "skip -1 to 1")) %>%
  mutate(earning = if_else( (Total-OUClose)<0, 95, if_else(Total==OUClose, 0, -100) ) )
...
ggplot(df2, aes(x=Diff_dec_idx, y = profit)) +
  ...
  xlab("Score Difference") +
  ylab("Profit ($)") + 
  ggtitle("Placing Bet On Under OUClose for All Games")
```
...
ggplot(df, aes(x=index, y=..count..)) + 
  geom_bar() +
  ggtitle("Counts for 1st and 2nd game score comparsion")
